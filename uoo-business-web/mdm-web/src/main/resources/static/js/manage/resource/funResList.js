...
//初始化功能资源表格
function initFuncResTable(keyWord){
    table = $("#funResTable").DataTable({
        'destroy':true,
        'searching': false,
        'autoWidth': true,
        'ordering': true,
        ...
        'columns': [
            ...
            { 'data': "funcName", 'title': '功能名称', 'className': 'row-fName',
                'render': function (data, type, row, meta) {
                    return "<a href='javascript:void(0);' onclick='setFunResInfo("+row.funcId+")'>"+row.funcName+"</span>";
                }
            },
            { 'data': "funcCode", 'title': '功能编码', 'className': 'row-fNum'},
...
function search () {
    ...
    initFuncResTable(query);
}

function setFunResInfo(id){
    window.location.href = "funResInfo.html?id="+id;
}
