...
//初始化文件资源
function initFileResTable(search){
    table = $("#fileResTable").DataTable({
        'destroy':true,
        'searching': false,
        'autoWidth': false,
        'ordering': true,
        ...
        'columns': [
            ...
            { 'data': "fileName", 'title': '文件名称', 'className': 'row-fName',
                'render': function (data, type, row, meta) {
                    return "<a href='javascript:void(0);' onclick='setFileResInfo("+row.fileId+")'>"+row.fileName+"</span>";
                }
            },
        ...
        'ajax': function (data, callback, settings) {
            ...
            param.search = search;
            $http.get('/sysFile/getSysFilePage', param, function (result) {
                var returnData = {};
...
function search () {
    ...
    initFileResTable(query);
}

function setFileResInfo(id){
    window.location.href = "fileResInfo.html?id="+id;
}
