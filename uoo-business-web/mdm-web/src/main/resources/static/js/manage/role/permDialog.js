...
var permList = roleFrame.permList;
...
function initRoleTree () {
    ...
    $http.get('/system/sysPermission/listPage?pageNo=1&pageSize=1000&keyWord=', {}, 
    function (data) {
        allPerms = [];
        for(var i=0;i < data.records.length;i++){
            allPerms.push({"id":data.records[i].permissionId, "name":data.records[i].permissionName,"code":data.records[i].permissionCode});
        }
        $.fn.zTree.init($("#roleTree"), roleTetting, allPerms);
        autoCheck();
    }, function (err) {
