...
function deleteData(){
	...
	parent.layer.confirm('确定删除?', {
        ...
        $http.post('/region/politicalLocation/deletePoliticalLocation',JSON.stringify({"locId":id}),function(data){
        	toastr.success('操作成功');
...
function get(id){
	$http.get('/region/politicalLocation/getPoliticalLocation/id='+id,{},function(data){
		curUpId=data.parentLocId
		loadLocType();
		initData(data);
	}) ;
...
function saveRegion(){
	...
	$http.post('/region/politicalLocation/updatePoliticalLocation',JSON.stringify(serializeObject($('#regionForm'))),function(data){
		toastr.success('操作成功');
