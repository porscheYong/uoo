 ...
 *   https://datatables.net/download/#bs/dt-1.10.16/b-1.5.1/cr-1.4.1
 *
 * Included libraries:
 *   DataTables 1.10.16, Buttons 1.5.1, ColReorder 1.4.1
 */
...
(function( $, window, document, undefined ) {
	...
	DataTable.ext = _ext = {
		 ...
		 *  @type string
		 */
		build:"bs/dt-1.10.16/b-1.5.1/cr-1.4.1",
	...
	var attach = function( container, buttons ) {
		...
		var clickHandler = function ( e ) {
			...
			if ( !$(e.currentTarget).hasClass('disabled') && api.page() != e.data.action ) {
				...
				// *******
				var orglist = orgTable.rows({page:'current'}).data();
				var curOrgDataId = [];
				var deledId = [];
				var addedId = [];
				for(var i=0;i<orglist.length;i++){
					curOrgDataId.push(orglist[i].orgId);
				}
				for(var i=0;i<isAdd.length;i++){
					if(curOrgDataId.indexOf(isAdd[i]) != -1){
						$("#org_"+isAdd[i]).attr("disabled","true");
						addedId.push(isAdd[i]);
					}
				}
				for(var i=0;i<isDel.length;i++){
					if(curOrgDataId.indexOf(isDel[i]) != -1){
						$("#org_"+isDel[i]).removeAttr("disabled");
						$("#org_"+isDel[i]).parent().parent().removeClass('select');
						$("#org_"+isDel[i]).removeAttr("checked");   
						deledId.push(isDel[i]);
					}
				}
				for(var i=0;i<deledId.length;i++){
					isDel.splice(isDel.indexOf(deledId[i]),1);
				}
				for(var i=0;i<addedId.length;i++){
					isAdd.splice(isAdd.indexOf(addedId[i]),1);
				}
				// ******
