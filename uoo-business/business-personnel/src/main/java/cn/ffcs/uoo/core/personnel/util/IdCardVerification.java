...
import cn.ffcs.uoo.core.personnel.vo.IdCardInfoVo;
...
public class IdCardVerification {
    /**身份证有效*/
    public static final String VALIDITY = "1000";
    /**位数不足*/
    ...
    public static boolean idCardValidate(String IDStr) {
        ...
        try {
            if (BaseUnitConstants.ENTT_STATE_ACTIVE.equals(IDCardValidate(IDStr))){
                return true;
            }
     ...
     */
    private static Pattern  pIsNumeric = Pattern.compile("[0-9]+");
    private static boolean isNumeric(String strnum) {
        Matcher isNum = pIsNumeric.matcher(strnum);
        if (isNum.matches()) {
            return true;
        } else {
            return false;
     ...
     * @return true, 符合; false, 不符合。
     */
    private static Pattern pIsDate = Pattern.compile(
            "^((\\d{2}(([02468][048])|([13579][26]))[\\-\\/\\s]?((((0?[13578])|(1[02]))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])))))|(\\d{2}(([02468][1235679])|([13579][01345789]))[\\-\\/\\s]?((((0?[13578])|(1[02]))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\\-\\/\\s]?((0?[1-9])|(1[0-9])|(2[0-8]))))))?$");
    public static boolean isDate(String strDate) {
        Matcher m = pIsDate.matcher(strDate);
        if (m.matches()) {
            return true;
        } else {
            return false;
    ...
    public static Object parseCertificateNo(String certificateNo) {

        if(!IdCardVerification.idCardValidate(certificateNo)){
            return ResultUtils.error(EumPersonnelResponseCode.CERT_ERROR);
        }
...
//        }
        ...
        idCardInfoVo.setBirthday(birthday);
        ...
        idCardInfoVo.setAge(yearAge - monthFloor);

        //籍贯
        idCardInfoVo.setNtivePlace(NtivePlaceUtil.getNtivePlace(certificateNo.substring(0,6)));

        return ResultUtils.success(idCardInfoVo);
    }
...
//        IdCard = IdCard.toUpperCase();
        ...
        IdCardVerification.parseCertificateNo("61082120061222612X");
    }
